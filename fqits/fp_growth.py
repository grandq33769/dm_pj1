...
from fqits.base import FrequentItemSet, transform
...
class FPgrowth(FrequentItemSet):
    ...
    def __init__(self, data, min_sup):
        super(FPgrowth, self).__init__(data, min_sup)
        counts = self.count(transform(self.org))
        self.fpt = FPtree(counts)

    def run(self):
        num = 1
        one = int(len(self.data) * 0.05)
        for trans in self.data:
            self.fpt.add(trans)
            num += 1
            if num % one == 0:
                log.info('Finish Processing {0:.0f}% of transactions ....'
                         .format(num * 5 / one))
...
class FPnode(Node):
    ...
    def trace(self):
        ...
        Returns:
            r_node(FPnode):
                New node consist the path from self node to
                root node(Bottom Up) with self node count
            r_dict({item:count}):
...
class FPtree(object):
    ...
    def link(self, node, childs=False):
        '''
        Args:
            node(FPnode): Node will link with the tree responding to its value
            childs(boolean): Link with its childrens
        '''
        if isinstance(node, FPnode):
            ...
            if childs:
                for child in node.childs:
                    self.link(child, True)

    def _link(self, node):
        '''
        Args:
            node(FPnode):
             FPnode will link with the tree responding to its value
        '''
        try:
...
if __name__ == '__main__':
    ...
    FP = FPgrowth(DATA1, 0.10)
    RESULT = FP.run()

    for fqits, time in RESULT.items():
        print(fqits, time)
